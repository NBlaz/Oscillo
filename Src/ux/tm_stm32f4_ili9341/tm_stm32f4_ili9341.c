...
void TM_ILI9341_Init(SPI_HandleTypeDef* lcdSPI_in) {
	
	lcdSPI=lcdSPI_in;

	GPIO_InitTypeDef LCD_GPIO_Init = {0};
	/*Configure GPIO pin : ILI9341_CS_PIN */
	LCD_GPIO_Init.Pin = ILI9341_CS_PIN;
	LCD_GPIO_Init.Mode = GPIO_MODE_OUTPUT_PP;
	LCD_GPIO_Init.Pull = GPIO_NOPULL;
	HAL_GPIO_Init(ILI9341_CS_PORT, &LCD_GPIO_Init);

	/*Configure GPIO pin : ILI9341_CS_PIN */
	LCD_GPIO_Init.Pin = ILI9341_WRX_PIN;

	HAL_GPIO_Init(ILI9341_WRX_PORT, &LCD_GPIO_Init);
...
void TM_ILI9341_SendCommand(uint8_t data) {
	...
	HAL_SPI_Transmit(lcdSPI, &data, 1, 0x0000FFFFU);
	ILI9341_CS_SET;
}
...
void TM_ILI9341_INT_Fill(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color) {
	...
	lcdSPI->Init.DataSize = SPI_DATASIZE_16BIT;
	if (HAL_SPI_Init(lcdSPI) != HAL_OK)
	{
    	Error_Handler();
  	}
  	while (HAL_SPI_GetState(lcdSPI)==HAL_SPI_STATE_BUSY);
  	
	/* Send first 65535 bytes, SPI MUST BE IN 16-bit MODE */

	HAL_SPI_TransmitCircle_DMA(lcdSPI, color, 2*((pixels_count > 0xFFFF) ? 0xFFFF : pixels_count));
	
	while (HAL_SPI_GetState(lcdSPI)==HAL_SPI_STATE_BUSY || HAL_DMA_GetState(lcdSPI->hdmatx)==HAL_DMA_STATE_BUSY);
	
	/* Check again */
	if (pixels_count > 0xFFFF) {
		/* Send remaining data */
		HAL_SPI_TransmitCircle_DMA(lcdSPI, color, 2*(pixels_count - 0xFFFF));
