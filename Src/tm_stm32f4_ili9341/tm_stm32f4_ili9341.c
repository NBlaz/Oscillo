...
void TM_ILI9341_INT_Fill(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color) {
	...
	/* Send first 65535 bytes, SPI MUST BE IN 16-bit MODE */
  	uint16_t colordata[0xFFFF]={color};
	//TM_SPI_DMA_SendHalfWord(ILI9341_SPI, color, (pixels_count > 0xFFFF) ? 0xFFFF : pixels_count);


	HAL_SPI_Transmit_DMA(lcdSPI, (uint8_t *)colordata, 2*((pixels_count > 0xFFFF) ? 0xFFFF : pixels_count));
	while (HAL_SPI_GetState(lcdSPI)==HAL_SPI_STATE_BUSY);

	/* Check again */
	if (pixels_count > 0xFFFF) {
		/* Send remaining data */
		HAL_SPI_Transmit_DMA(lcdSPI, (uint8_t *)colordata, 2*(pixels_count - 0xFFFF));
